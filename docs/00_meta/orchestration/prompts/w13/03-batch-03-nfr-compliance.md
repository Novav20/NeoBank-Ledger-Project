...
## Objectives
1.  **Iterative Processing**: Analyze exactly **9 unprocessed sources** from `docs/02_analysis/research/w13/batch-03/` per turn.
...
-   **Limit**: Process ONLY 9 sources per turn.
...
1.  **Selection**: Identify 9 files in the batch-03 folder where `processed: false`.
2.  **Analysis**:
...
3.  **Synthesis**:
    ...
    -   If the report exists, add an `### UPDATE-XX` block (following the Delta Update protocol) to integrate these 9 sources into the existing synthesis.
4.  **Metadata Writeback**: Update the source files' YAML block to reflect they are now `processed`.
5.  **Completion**: Stop after 9 sources and summarize which authors were processed. **Do not continue until the user validates this block.**
